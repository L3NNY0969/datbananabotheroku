...
bot.db = AsyncIOMotorClient(os.environ.get("MONGODB"))
...
async def on_message_edit(before, after):
    if modlog_check(before.guild.id):
        x = await bot.db.datbananabot.modlog.find_one({"id": str(before.guild.id)})
        try:
...
async def on_member_join(member):
    ...
    if channel is False:
        ...
        await lol.send(x['message'].replace('{name}', member.name).replace('{mention}', member.mention).replace('{members}', strlen(member.guild.members)))
