        ...
        if not ctx.voice_client.is_playing():
            ...
            try:
                ctx.voice_client.play(
                    player, after=self.next_song if self.next_song else None)
            except discord.Forbidden:
                return await ctx.send("I don't have permissions to play in this channel.")
            em = discord.Embed(color=discord.Color(
                value=0x00ff00), title=f"Playing")
            em.description = player.title
            em.set_author(name=ctx.author.name, icon_url=ctx.author.avatar_url)
            em.add_field(
                name='Length', value=f"{int(int(player.get_duration())/60)}:{int(player.get_duration()) - int(int(player.get_duration())/60)*60}")
                ...
                while True:
                    ...
                    if reaction.emoji == ":stop_button:":
                        ...
                        embed = discord.Embed(color=discord.Color(
                            value=0x00ff00), title='Music Player Help')
            ...
            em = discord.Embed(color=discord.Color(
                value=0x00ff00), title='Added to queue!')
            em.description = f"Song: {to_play.title}"
            em.add_field(name='Position in Queue', value=len(self.queue) - 1)
            em.add_field(
                name='Length', value=f"{int(int(to_play.get_duration())/60)}:{int(to_play.get_duration()) - int(int(to_play.get_duration())/60)*60}")
            em.set_author(
                name=f"Played by: {ctx.author.name}", icon_url=ctx.author.avatar_url)
